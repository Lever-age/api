...
var campaignsByCandidate = require('../controllers/candidates').campaignsByCandidate;

describe('campaignsByCandidate', () => {
  var req = {
    params: {}
  };
  it('sends the retrieved object as a response', (done) => {
    var backend = {
      fetchByCandidate: (id, cb) => cb(undefined, 'response content')
    };
    var res = {
      json: (content) => {
    ...
    campaignsByCandidate({backend: backend}, req, res);
  });
  it('logs and sends error response', (done) => {
    var backend = {
      fetchByCandidate: (id, cb) => cb({name: 'test error', message: 'error content'})
    };
    var logger = {
      log: (level, format, name, message) => {
        expect(level, 'incorrect log level specified').to.equal('error');
        expect(format, 'unexpected format string').to.equal('campaignsByCandidate: %s: %s');
    ...
    campaignsByCandidate({backend: backend, logger: logger}, req, res);
  });
});
